  ...
  $(function ($) {
    if (cc.sections.length) {
  	...
  	if( /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) ) {
    	jQuery('.header-only-mobile img').css('display','block');
	}
	...
	jQuery(".option-selector.option-selector--swatch .option-selector__btns input[type=radio]").click(function(){
      	let that = jQuery(this);
      	console.log(that.val());
    });
	jQuery('.cart-item__not-image .cart-item__column  ').find('.small-text').each(function(){
        let that = this;
        jQuery(this).parent().find('.line-item-property').slice(-3).css('display','none');
    });
	function myFunction(x) {
      if (x.matches) {
        jQuery('#section-id-template--15011635429433__1652862779afda793d .gallery__row').slick({
          dots: false,
          infinite: false,
          autoplay: true,
          autoplaySpeed: 3000,
    ...
    var x = window.matchMedia("(max-width: 900px) and (min-width: 768px)")
    myFunction(x)
    x.addListener(myFunction)
    
    setTimeout(() => {
        if( jQuery('.shopify-payment-button__button--hidden').css('visibility') == 'hidden' )  { 
            jQuery('.shopify-payment-button__button--hidden').css('display','none'); 
        }
    }, "2000")
    
    if ($(window).width() < 1000) {
    	$(document).on('click', '.navigation__item', function() {
            jQuery('.mobile-navigation-drawer').animate({
                scrollTop: jQuery(".navigation__item.featured-link").offset().top
            }, 500);
		});
    }
